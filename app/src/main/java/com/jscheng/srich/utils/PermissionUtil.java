...
public class PermissionUtil {
    ...
    public static boolean checkPermissionsAndRequest(Context context, String[] permissions, int requestCode, String hint) {
        List<String> notGrantPermissions = isPermissionsAllGranted(context, permissions);
        if(notGrantPermissions.isEmpty()){
            return true;
        }
        if (judgePermission(context, permissions)) {
            showPermissionAlterDialog(context, hint);
        } else {
